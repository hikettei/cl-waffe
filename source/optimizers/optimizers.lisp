...
(defoptimizer Momentum (params &key (momentum 0.9) (lr 1e-3))
  ...
  :update (()
	   ...
	   (dotimes (i (length (self params)))
	     ...
	     ; w(n+1) = w(n) + v(n+1)
	     (setf (nth i (self velocities)) (data (!sub (!mul (self momentum) (nth i (self velocities)))
							 (!mul (self lr) (grad (nth i (self params)))))))
	     (mgl-mat:copy! (data (!add (nth i (self velocities))
					(nth i (self params))))
...
(defoptimizer AdaGrad (params &key (lr 1e-3) (epsilon 1e-7))
  ...
  :update (()
	   ...
	   (dotimes (i (length (self params)))
	     ...
	     (setf (nth i (self h)) (data (!add (nth i (self h))
						(!mul (grad (nth i (self params)))
						      (grad (nth i (self params)))))))
	     (mgl-mat:copy! (data (!sub (data (nth i (self params)))
...
(defoptimizer RMSProp (params &key (lr 1e-3) (epsilon 1e-7) (decay-rate 0.99))
  ...
  :update (()
	   (if (equal (self h) T)
		 ...
		 (push 0 (self h))))
	   (dotimes (i (length (self params)))
             (setf (nth i (self h)) (data (!mul (nth i (self h)) (self decay-rate))))
	     (setf (nth i (self h)) (data (!add (nth i (self h))
...
(defoptimizer Adam (params &key (lr 1e-3) (epsilon 1e-7) (beta1 0.9) (beta2 0.999))
  ...
  :update (()
	   ...
	   (let ((lr-t (!mul (self lr) (!div (!sqrt (!sub 1.0 (!pow (self beta2) (self i))))
	     ...
	     (dotimes (i (length (self params)))
	       (setf (nth i (self m)) (data (!add (nth i (self m))
						  (!mul (!sub 1 (self beta1))
							(!sub (grad (nth i (self params)))
							      (nth i (self m)))))))
	       (setf (nth i (self v)) (data (!add (nth i (self v))
						  (!mul (!sub 1 (self beta2))
							(!sub (!pow (grad (nth i (self params))) 2)
							      (nth i (self v)))))))
	       (setf (data (nth i (self params))) (data (!sub (nth i (self params))
							      (!mul lr-t (!div
									  (nth i (self m))
